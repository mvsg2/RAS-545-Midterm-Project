...
from robot_utilities import move_to_home
...
def Agent01_capture_image():
    ...
    ap.add_argument("--out", default="maze_corners_overlay.png", help="Path to save overlay visualization")
    ...
    ap.add_argument("--json-out", type=str, default="camera_capture_corners.json", help="Corners JSON output path (default: camera_capture_corners.json)")
    ap.add_argument("--save-raw", type=str, default="camera_capture.png", help="Optional path to save the raw captured frame (default: camera_capture.png)")
    args = ap.parse_args()

    dev = Dobot(port=PORT)
    move_to_home(device=dev)
    dev.close()
