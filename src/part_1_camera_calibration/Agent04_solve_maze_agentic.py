...
class GeminiLLM:
    ...
    def __init__(self, api_key, model, grid, start, end):
        self.api_key = api_key
        self.model = model

        self.grid_data : List[List[Cell]] = grid
        self.start_point = start
        self.end_point = end
        # self.start_color = input("What is the start color? (red/green): ").lower().strip()
        # if self.start_color not in ["red", "green"]:
        #     raise ValueError("Color given is invalid")
        # if self.start_color=="red":
        #     self.end_color = "green"
        # else:
        #     self.end_color = "red"
        self.PROMPT = """You are a path planner for grid mazes. Solve the given maze satisfying the following:

        Rules the path MUST satisfy:
        - The grid is rows x cols. Each cell has value 0 or 1.
        - Movement is 4-connected only: Up/Down/Left/Right (no diagonals).
        - The starting point is the cell with start color and the ending point is the cell with end color as received from the input.
        - Go only from the starting point and finish at the ending point. And nothing else. 
        - Traversable cells must have value 1.
        - The start and end cells may be 0 and are allowed.
        - The path must start at the exact start cell and end at the exact end cell.
        - Output strictly valid JSON with *no* extra commentary, using this schema:
        {
            "path": [(row0, col0), (row1, col1), ..., (rowN, colN)]
        }
        - Use this List[Tuple[int, int]] format only for the output and nowhere else. Only this should be your output.
        - Do not print any codes, comments, etc. Only return the output as mentioned.
        - The path must include both endpoints, in order, and every step must move by Manhattan distance exactly 1.
        - There could be multiple paths from the start to the goal. The path must be the most optimum shortest path. 
        """

        self.client = genai.Client(api_key=self.api_key)
        
        # self.contents = [
        #     types.Content(role="user", parts=[types.Part.from_text(text=self.PROMPT)]),
        #     types.Content(role="user", parts=[types.Part.from_text(text=json.dumps(self.grid_data))])
        # ]
        # self.contents = [(self.grid_data, self.start_point, self.end_point), self.PROMPT]
        # self.contents = {
        #     "role":"user",
        #     "parts":[
        #         {"text":json.dumps({
        ...
        self.payload = {
            "grid":self.grid_data,
            "start_px":self.start_point,
            "end_px":self.end_point,
        }
        self.contents = types.Content(
            role="user",
            parts=[
                types.Part.from_text(text=str(self.payload)), # payload string containing the grid and pixel coords of start & end points 
                types.Part.from_text(text=self.PROMPT) # prompt string containing the input text from the user
            ]
        )

        self.config = types.GenerateContentConfig(temperature=0.01, top_p=0.9)
...
def llm_path(api_key:str, model:str, grid, start_px, end_px): # using Google Gemini-2.5 Flash
    llm = GeminiLLM(api_key=api_key, model=model, grid=grid, start=start_px, end=end_px)
    response_text = llm.get_response()
    def extract_json(text):
        st = text.find("{")
        en = text.find("}")
        if st==-1 or en==-1 or en<=st:
            raise json.JSONDecodeError("No JSON object could be detected", text, 0)
        return text[st:en+1]
    json_txt = extract_json(response_text)
    path_dict = ast.literal_eval(json_txt)
...
def Agent04_maze_solver():
    ap = argparse.ArgumentParser(description="Solve a grid maze from JSON and draw the path.")
    ap.add_argument("--json_path", default= "result.json" , help="Input maze JSON path.")
    ...
    # LLM solution
    path = llm_path(
        api_key="AIzaSyCG8yBgNo1y6QScvUK5H6hVmNrdpbPOUEk",
        # model="gemini-2.5-flash",
        model="gemini-2.5-pro",
        # model="gemini-robotics-er-1.5-preview",  # robotics model gave a really good solution once!
